...
```{r}
Survey <- read.csv(
  'https://www.lock5stat.com/datasets2e/StudentSurvey.csv',
  na.strings=c('',' '))
```
...
Survey <- Survey %>% 
  mutate(Year = 
           fct_relevel(Year, "FirstYear", "Sophomore", "Junior", "Senior"))
str(Survey)
```
...
```{r}
SurveyGroups %>% pivot_wider(names_from=Year,values_from=n)
```
...
```{r}
cbind(A,B) #this does not actually match along Name/First.Name
left_join(A,B,by=join_by(Name == First.Name))
```
...
```{r}
#cbind(A,B) #this does not work with different row numbers
right_join(A,B,by=join_by(Name == First.Name))
```
Since cbind did not work and right_join was very straightforward, the latter was the easiest and cbind is more likely to have an error.
...
FlagTemp <- read_csv(
  ...
  pivot_longer(cols=4:34,names_to="Day") %>%
  drop_na() %>%
  mutate(Date = ymd(paste(Year,"-",Month,"-",Day)))
FlagTemp05 <- FlagTemp %>% filter(Year==2005)
head(FlagTemp05)
ggplot(data=FlagTemp05,aes(x=Date,y=value)) +
  geom_line() +
  labs(x="Date",
       y="Maximum Temperature",
       title="2005 Flagstaff Max Temperatures")
```
...
head(FlagTemp13)
ggplot(data=FlagTemp13,aes(x=Date,y=Average)) +
  geom_line() +
  labs(x="Date",
       y="Maximum Temperature",
       title="2005 Flagstaff Max Temperatures")
...
personID <- filter(Customers,Name == aubrey)$PersonID
cardID <- filter(Cards,PersonID == personID)$CardID
Cards$Exp_DateTime[Cards$CardID == cardID] <- 
  ymd_hms("2019-10-15 4:28:21 PM")
Cards
newcard <- '5544339988675849'
Cards <- Cards %>% 
  add_row(CardID = newcard, 
          PersonID = personID, 
          Issue_DateTime = ymd_hms('2019-10-16 0:00:00'),
...
```{r}
card <- newcard
retailer <- 'Kickstand Kafe'
datetime <- ymd_hms('2019-10-16 14:30:21')
amount <- 4.98
retailid <- filter(Retailers,Name == retailer)$RetailID
Transactions <- Transactions %>% 
  add_row(CardID = card, 
          RetailID = retailid, 
          DateTime = datetime, 
